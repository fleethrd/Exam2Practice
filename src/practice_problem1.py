...
class Box(object):
    ...
    def append_string(self, additional_contents):
        ...
        clipped = ''
        available = self.volume - len(self.contents)
        if available > len(additional_contents):
            for k in range(len(additional_contents)):
                self.contents = self.contents + additional_contents[k]
        else:
            for k in range(available):
                self.contents = self.contents + additional_contents[k]
            for k in range(available, len(additional_contents)):
    ...
    def reset(self):
        ...
        self.volume = self.origin_volume
        self.contents = self.origin_contents
